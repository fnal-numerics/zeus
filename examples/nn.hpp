...
struct NeuralNet {
  ...
  NeuralNet(const std::array<double, In>& x_, const std::array<double, Out>& y_)
    : x_host(In, 1), x_dev(In, 1), y_host(Out, 1), y_dev(Out, 1)
  {
    ...
    x_ptr = x_dev.deviceData();
    y_ptr = y_dev.deviceData();
  }

  // templated sigmoid, works on double or DualNumber
  template <typename T>
  __host__ __device__ static T
  sigmoid(T t)
  {
  ...
  operator()(const std::array<T, P>& theta) const
  {
    size_t idx = 0;
    T hidden[H];
