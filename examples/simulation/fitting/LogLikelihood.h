...
public:
  ...
  operator()(std::array<T, 4> const& theta) const
  {
    using ::lgamma; // device lgamma(double); ADL picks dual::lgamma for
                    // DualNumber
    using ::log;    // same
    using ::pow;    // device intrinsics for double; ADL picks dual::pow for
                    // DualNumber
