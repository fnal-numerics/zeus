...
namespace parallel {
  ...
  if (tile.thread_rank() == 0) {
    #pragma unroll
    for (int j = 0; j < DIM; ++j) g_out[j] = scratch[j];
  }
  tile.sync();
}
...
template <typename Function, std::size_t DIM = fn_traits<Function>::arity, int TS>
__global__ void optimize_tiles(
    Function f,
    const double lower,
    const double upper,
    ...
    bool save_trajectories = false)
{
  ...
  static_assert(
    std::is_same_v<decltype(std::declval<Function>()(
                     std::declval<std::array<dual::DualNumber, DIM>>())),
                   dual::DualNumber>,
    "Objective must be templated: template<class T> T f(const std::array<T,DIM>&)");
  ...
  if (tile_global_id >= N) return;

  // per-tile shared memory layout:
  // [ x_shared (DIM) | grad_scratch (DIM) | done_flag (1 as int) ]
  extern __shared__ double smem[];
  const int per_tile_doubles = 2 * DIM + 1; // last double used as an int flag
  ...
  grad_ad_tile<Function, DIM, TS>(f, x_shared, grad_scratch, tile, grad_scratch);
  if (tile.thread_rank() == 0) {
    for (int d = 0; d < DIM; ++d) g_arr[d] = grad_scratch[d];
  }
  tile.sync();

  double bestVal = 0.0;
  if (tile.thread_rank() == 0) bestVal = f(x_arr);
  ...
  for (; iter < MAX_ITER; ++iter) {
    ...
    if (tile.thread_rank() == 0) {
      if (atomicAdd(&d_stopFlag, 0) != 0) {
        write_result<DIM>(r, 2, f(x_arr), x_arr.data(),
                          util::calculate_gradient_norm<DIM>(g_arr), iter, tile_global_id);
        *done_flag = 1;
      }
    }
    tile.sync();
    ...
    if (tile.thread_rank() == 0) {
      ...
      if (alpha == 0.0) {
        printf("Alpha is zero, using fallback alpha=1e-3 (iter=%d, idx=%d)\n", iter, tile_global_id);
        alpha = 1e-3;
      }
      for (int d = 0; d < DIM; ++d) {
    ...
    grad_ad_tile<Function, DIM, TS>(f, x_shared, grad_scratch, tile, grad_scratch);
    if (tile.thread_rank() == 0) {
      for (int d = 0; d < DIM; ++d) g_new[d] = grad_scratch[d];
      ...
      const double delta_dot= util::dot_product_device(delta_x, delta_g, DIM);
      util::bfgs_update<DIM>(&H, delta_x, delta_g, delta_dot, &Htmp);

      if (fnew < bestVal) {
        bestVal = fnew;
        for (int d = 0; d < DIM; ++d) { x_arr[d] = x_new[d]; g_arr[d] = g_new[d]; }
      }

      const double grad_norm = util::calculate_gradient_norm<DIM>(g_arr);
      if (!isfinite(grad_norm) || !isfinite(fnew)) {
        write_result<DIM>(r, 5, fnew, x_arr.data(), grad_norm, iter, tile_global_id);
        *done_flag = 1;
      } else if (grad_norm < tolerance) {
        const int oldCount = atomicAdd(&d_convergedCount, 1);
        const double fcurr = f(x_arr);
        write_result<DIM>(r, 1, fcurr, x_arr.data(), grad_norm, iter, tile_global_id);
  ...
  if (tile.thread_rank() == 0) {
    if (!*done_flag && iter == MAX_ITER) {
      write_result<DIM>(r, 0, f(x_arr), x_arr.data(),
...
static size_t smem_for_block(int block_threads) {
       ...
       Function f)
{
  ...
  for (int cand : {256, 192, 160, 128, 96, 64, 32}) {
    if (cand < TS) continue;           // block must be ≥ tile
    if (cand % 32) continue;           // multiple of warp
    blockThreads = cand; break;
  }
...
const size_t shmemBytes = smem_for_block<(int)DIM, TS>(blockThreads);
  ...
  dim3 optGrid( static_cast<unsigned>((N + tilesPerBlock - 1) / tilesPerBlock) );
  dbuf deviceResults;
  try { deviceResults = dbuf(N); }
  catch (const cuda_exception<3>&) { Result<DIM> r{}; r.status = 3; return r; }

  result_buffer<DIM> d_results;
  try { d_results = result_buffer<DIM>(N); }
  catch (const cuda_exception<3>&) { Result<DIM> r{}; r.status = 3; return r; }
  ...
  optimize_tiles<Function, (int)DIM, TS>
    <<<optGrid, optBlock, shmemBytes>>>(
      ...
      (int)N,
      MAX_ITER, requiredConverged, tolerance,
      d_results.data(),
      states,
      save_trajectories);
