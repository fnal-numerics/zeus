...
public:
  __device__ DeviceMatrix(std::size_t rows, std::size_t cols);  ///< Allocate rows×cols matrix on device heap
  ...
  __device__ T& operator()(std::size_t i, std::size_t j);  ///< Access element at (i,j) in row-major order
  __device__ const T& operator()(std::size_t i, std::size_t j) const;  ///< Access element at (i,j) in row-major order (const)
  __device__ void set(std::size_t i, std::size_t j, const T& x);  ///< Set element at (i,j)
};
