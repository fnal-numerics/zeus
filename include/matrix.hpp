...
private:
  void
  boundsCheck(std::size_t i, std::size_t j) const
  {
    if (i >= rows_ || j >= cols_)
...
public:
    ...
    : Matrix(rows, cols)
  {
    ...
    CUDA_CHECK(cudaMemcpy(
      device_data_, host_data_, expected * sizeof(T), cudaMemcpyHostToDevice));
  }
...
#else
    if (!rows_ || !cols_)
      return; // allow copying an empty matrix
  ...
  void
  set(std::size_t i, std::size_t j, const T& x)
  {
    boundsCheck(i, j);
    const std::size_t idx = i * cols_ + j;
    host_data_[idx] = x;
    CUDA_CHECK(
      cudaMemcpy(device_data_ + idx, &x, sizeof(T), cudaMemcpyHostToDevice));
  }
  ...
  void
  set(const T* host_ptr, std::size_t count)
  {
    const std::size_t expected = rows_ * cols_;
    if (count != expected)
