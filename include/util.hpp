...
namespace util {
                     ...
                     const zeus::Result<DIM>& best)
  {
    ...
    if (file_empty) {
      outfile << "fun\trun\tN\tkernel"
              << "\tad_ms_per_call\tad_calls_per_thread_mean\tad_fraction\tad_"
                 "block95\tad_serialized"
              << "\tbfgs_ms_per_call\tbfgs_calls_per_thread_mean\tbfgs_"
                 "fraction\tbfgs_block95\tbfgs_serialized"
              << "\tclaimed\tactual\tsurrender\tstopped\tidx\tstatus\tbfgs_"
                 "iter\tpso_iter\ttime\terror\tfval\tnorm";
      for (int i = 0; i < dim; i++)
        outfile << "\tcoord_" << i;
      outfile << std::endl;
    ...
    outfile << fun_name << "\t" << run << "\t" << N << "\t" << ms_opt << "\t"
            ...
            << "\t" << best.ad.fraction_of_thread << "\t" << best.ad.block95
            << "\t" << best.ad.serialized << "\t" << best.bfgs.ms_per_call
            << "\t" << best.bfgs.calls_per_thread_mean << "\t"
            << best.bfgs.fraction_of_thread << "\t" << best.bfgs.block95 << "\t"
            << best.bfgs.serialized << "\t" << best.c.claimed << "\t"
            << best.c.actual << "\t" << best.c.surrendered << "\t"
            << best.c.stopped << "\t" << best.idx << "\t" << best.status << "\t"
            << max_iter << "\t" << pso_iter << "\t" << time_seconds << "\t"
            << std::scientific << error << "\t" << best.fval << "\t"
            << best.gradientNorm << "\t";
    std::cout << "claimed: " << best.c.claimed << "\tactual : " << best.c.actual
              << "\tsurrendered: " << best.c.surrendered
              << "\tstopped: " << best.c.stopped << "\n";
    for (int i = 0; i < dim; i++) {
  ...
  template <size_t DIM>
  Convergence
  dumpDataToFile(size_t N,
                 zeus::Result<DIM>* h_results,
                 std::string fun_name,
                 const int PSO_ITER,
                 const int run)
  {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      if (h_results[i].status == 1) {
        ...
        // outfile << 1 << tab;
        double error =
          calculateEuclideanError(fun_name, h_results[i].coordinates, DIM);
        if (error < 0.5) {
          actually_converged++;
        }
