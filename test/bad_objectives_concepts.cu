...
/// that the concept constraints properly catch common user errors at compile
/// time.
...
struct WrongArgType {
  ...
  { // Should be: const std::array<T, DIM>&
    return x * x + y * y;
  }
};
...
instantiate_not_templated()
{
  ...
  auto result = zeus::Zeus(
    NotTemplated{}, -5.0, 5.0, 2, 10, 5, 1, "not_templated", 0.01, 42, 1);
}
...
instantiate_wrong_return()
{
  ...
  auto result = zeus::Zeus(
    WrongReturnType{}, -5.0, 5.0, 2, 10, 5, 1, "wrong_return", 0.01, 42, 1);
}
...
instantiate_wrong_args()
{
  ...
  auto result = zeus::Zeus(
    WrongArgType{}, -5.0, 5.0, 2, 10, 5, 1, "wrong_args", 0.01, 42, 1);
}
