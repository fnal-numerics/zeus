...
TEST_CASE("NonNull: Construction from valid pointer", "[NonNull][host]")
{
  ...
  REQUIRE_NOTHROW([&]() { util::NonNull<double*> nn(ptr); }());
}
...
TEST_CASE("NonNull: Construction from nullptr throws", "[NonNull][host]")
{
  ...
  REQUIRE_THROWS_AS([&]() { util::NonNull<double*> nn(null_ptr); }(),
...
TEST_CASE("NonNull: Copy construction with convertible types",
          "[NonNull][host]")
{
  double value = 7.0;
  double* ptr = &value;
...
__global__ void
kernel_struct_access(util::NonNull<TestData*> data,
                     double* out_val,
                     int* out_count)
{
  if (threadIdx.x == 0) {
...
TEST_CASE("NonNull: Usage in CUDA kernel - array operations",
          "[NonNull][device][gpu]")
{
  const int size = 10;
  double h_input[size];
...
TEST_CASE("NonNull: Struct member access in CUDA kernel",
          "[NonNull][device][gpu]")
{
...
TEST_CASE("NonNull: Type conversion between compatible pointer types",
          "[NonNull][types]")
{
  ...
  REQUIRE_NOTHROW([&]() { util::NonNull<Base*> nn(base_ptr); }());
}
...
TEST_CASE("NonNull: Null pointer detection is guard-railed",
          "[NonNull][errors]")
{
  ...
  {
    REQUIRE_THROWS_AS([&]() { util::NonNull<double*> nn(invalid); }(),
                      std::invalid_argument);
  }
}
