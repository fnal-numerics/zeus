          ...
          "[pso][init]")
{
  ...
  // run initKernel<Function,DIM,StateType>
  pso::initKernel<util::Rastrigin<DIM>, DIM, curandStateXORWOW_t>
          ...
          "[pso][component][iter]")
{
  ...
  pso::iterKernel<util::Rastrigin<DIM>, DIM, curandStateXORWOW_t>
    <<<1, N>>>(util::Rastrigin<DIM>(),
               lower,
               upper,
               0.5, // inertia
...
TEST_CASE("pso::launch throws zeus::CudaError<3> when overflowing the memory")
{
  ...
  REQUIRE_THROWS_AS((pso::launch<Fn, DIM, curandStateXORWOW_t>(/*PSO_ITER*/ 10,
                                                               /*N*/ N,
                                                               /*lower*/ -2.0,
                                                               /*upper*/ 2.0,
                                                               ms0,
          ...
          "[pso][kernel-error]")
{
  ...
  curandStateXORWOW_t* states =
    bfgs::initializeStates<curandStateXORWOW_t>(N, 99, ms_rand);

  // This should reliably trigger cudaErrorIllegalInstruction (exception 4)
  REQUIRE_THROWS_AS(
    (pso::launch<Fn, DIM, curandStateXORWOW_t>(
          ...
          "[pso][malloc-error]")
{
  ...
  curandStateXORWOW_t* states =
    bfgs::initializeStates<curandStateXORWOW_t>(1, 42, ms_rand);
