...
struct BadObjective1 {
  __device__ double operator()(const std::array<int, DIM>& x) const {
    return 0.0;
  }
};
...
void instantiate_bad1() {
  ...
  bfgs::optimizeKernel<BadObjective1, DIM, 128>
    <<<1,128>>>(
      BadObjective1{},
      0.0, 1.0,
      d_pso,
      d_results,
      nullptr,
      /*N=*/1, /*MAX_ITER=*/1, /*reqConv=*/1, /*tol=*/1e-6,
      d_out,
      states
    );
}

void instantiate_bad2() {
  curandState* states   = nullptr;
  double*      d_pso     = nullptr;
  double*      d_results = nullptr;
  Result<DIM>* d_out     = nullptr;

  // this should pass the first check but trigger the static_assert (no DualNumber overload).
  bfgs::optimizeKernel<BadObjective2, DIM, 128>
    <<<1,128>>>(
      BadObjective2{},
      0.0, 1.0,
      d_pso,
