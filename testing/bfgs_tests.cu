...
struct Quadratic {
  __device__ double operator()(const std::array<double,1>& x) const {
    return x[0]*x[0];
  }
};

// Kernel to wrap util::line_search in 1D
__global__ void test_line_search(
    const double* x, const double* p, const double* g,
    double f0, double* out)
{
  out[0] = util::line_search<Quadratic,1>(
    f0, x, p, g, Quadratic()
  );
}

// Kernel to wrap gradient‐norm
__global__ void test_grad_norm(const double* g, double* out) {
  out[0] = util::calculate_gradient_norm<2>(g);
}

// Kernel to wrap identity initialization
__global__ void test_identity(double* H, int dim) {
  util::initialize_identity_matrix(H, dim);
}

// Kernel to wrap search direction (H * -g)
__global__ void test_search_dir(const double* H, const double* g, double* p) {
  util::compute_search_direction<3>(p, H, g);
}

// Kernel to wrap BFGS update
__global__ void test_bfgs_update(double* H,
                                 const double* dx,
                                 const double* dg,
                                 double dot) {
...
/*
under the Armijo rule with c1 = 0.3, for f(x)=x^2 at x0=1 and search direction p=–2, you get:
...
TEST_CASE("device line_search on x^2 from x=1 gives α=1", "[bfgs][line_search]") {
  ...
  cudaMalloc(&dX,  sizeof(hX)); cudaMalloc(&dP, sizeof(hP));
  cudaMalloc(&dG,  sizeof(hG)); cudaMalloc(&dOut, sizeof(double));
  ...
  cudaFree(dX); cudaFree(dP); cudaFree(dG); cudaFree(dOut);
}
...
TEST_CASE("device compute_search_direction uses H=I -> p=-g for 3D", "[bfgs][direction]") {
  constexpr int DIM = 3;
  ...
  cudaMalloc(&dH,sizeof(hH)); cudaMalloc(&dG,sizeof(hG)); cudaMalloc(&dP,sizeof(hP));
  ...
  cudaFree(dH); cudaFree(dG); cudaFree(dP);
}

TEST_CASE("device bfgs_update updates H correctly for a 2D step", "[bfgs][update]") {
  constexpr int DIM = 2;
  double hH[DIM*DIM]; 
  for (int i = 0; i < DIM; ++i)
    for (int j = 0; j < DIM; ++j)
  ...
  double *dH,*ddx,*ddg;
  cudaMalloc(&dH,sizeof(hH)); cudaMalloc(&ddx,sizeof(dx)); cudaMalloc(&ddg,sizeof(dg));
  cudaMemcpy(dH,hH,sizeof(hH),cudaMemcpyHostToDevice);
  ...
  cudaFree(dH); cudaFree(ddx); cudaFree(ddg);
}
...
struct Quad {
  ...
  __device__ T operator()(const std::array<T,N>& x) const {
