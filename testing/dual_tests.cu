...
__global__ void test_add(const double* a, const double* b, double* r, double* d) {
...
__global__ void test_sub(const double* a, const double* b, double* r, double* d) {
...
__global__ void test_mul(const double* a, const double* b, double* r, double* d) {
...
__global__ void test_div(const double* a, const double* b, double* r, double* d) {
    DualNumber A{a[0], a[1]};                    // load A = 6.0 + 2.0e
    DualNumber B{b[0], b[1]};                    // load B = 3.0 + 1.0e
    DualNumber R = A / B;                        // compute R.real = 6/3 = 2.0; R.dual = (2*3 - 6*1)/9 = 0.0
    r[0] = R.real;                               // store real part = 2.0
    d[0] = R.dual;                               // store dual part = 0.0
}

// Kernel for sin: tests sin(π/6 + 1e) = (0.5 + 0.866025e)
__global__ void test_sin(const double* x, const double* dx, double* r, double* d) {
    DualNumber D{x[0], dx[0]};                   // load D = π/6 ~ 0.523599 + 1.0e
    DualNumber R = dual::sin(D);                 // R.real = sin(π/6) = 0.5; R.dual = 1.0 * cos(π/6) = 0.866025
    r[0] = R.real;                               // store real part = 0.5
    d[0] = R.dual;                               // store dual part ≈ 0.866025
}

// Kernel for cos: tests cos(π/3 + 2e) = (0.5 − 1.73205e)
__global__ void test_cos(const double* x, const double* dx, double* r, double* d) {
    DualNumber D{x[0], dx[0]};                   // load D = π/3 ≈ 1.047198 + 2.0e
    DualNumber R = dual::cos(D);                 // R.real = cos(π/3) = 0.5; R.dual = −2.0 * sin(π/3) = −1.73205
    r[0] = R.real;                               // store real part = 0.5
    d[0] = R.dual;                               // store dual part ≈ −1.73205
}

// Kernel for exp: tests exp(1 + 3e) = (e + 3ee)
__global__ void test_exp(const double* x, const double* dx, double* r, double* d) {
    DualNumber D{x[0], dx[0]};                   // load D = 1.0 + 3.0e
    DualNumber R = dual::exp(D);                 // R.real = e ~ 2.71828; R.dual = 3.0 * e ~ 8.15484
    r[0] = R.real;                               // store real part ~ 2.71828
    d[0] = R.dual;                               // store dual part ~ 8.15484
}

// Kernel for sqrt: tests sqrt(4 + 1.5e) = (2 + 0.375e)
__global__ void test_sqrt(const double* x, const double* dx, double* r, double* d) {
...
__global__ void test_pow3(const double* x, const double* dx, double* r, double* d) {
    ...
    double* r, double* d)
{
    ...
    DualNumber R = dual::atan2(Y, X);            // R.real = π/4 ~ 0.785398; R.dual = (1*1 - 1*0)/2 = 0.5
...
TEST_CASE("operator+: (1+2e)+(3+4e)=4+6e", "[dual][add]") {
    ...
    cudaMalloc(&dR,sizeof(double)); cudaMalloc(&dD,sizeof(double));
...
TEST_CASE("operator pow^3: (2+1e)^3=8+12e", "[dual][pow]") {
