...
__global__ void valueKernel(const double* x, double* out, F<D> f) {
    std::array<double,D> xa;
    for(int i = 0; i < D; ++i) xa[i] = x[i];
    out[0] = f(xa);
}

template <template<int> class F, int D>
double run_value(const double x[D]) {
    double *dX, *dO, hO;
...
template <template<int> class F, int D>
__global__ void gradKernel(const double* x, double* g) {
    F<D> f;
    ...
    for(int i = 0; i < D; ++i) g[i] = ga[i];
}

template <template<int> class F, int D>
void run_grad(const double x[D], double out[D]) {
    double *dX, *dG;
    cudaMalloc(&dX, D*sizeof(double));
...
TEST_CASE("grad_Ackley@origin", "[fun][grad]")
{
    ...
    for(int i = 0; i < D; ++i) {
      std::cout << "got[" << i <<"]: " << got[i]  <<"expected[" << i << "]: " << expect[i] << std::endl;   
      REQUIRE(got[i] == Approx(expect[i]));
    }
}
