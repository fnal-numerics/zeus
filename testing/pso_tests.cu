...
template<typename T>
void copyDevice(const T* dptr, T* hptr, size_t n) {
    cudaMemcpy(hptr, dptr, n * sizeof(T), cudaMemcpyDeviceToHost);
}

TEST_CASE("pso::initKernel sets pBest & gBest for util::Rastrigin<2>", "[pso][init]") {
    ...
    pso::initKernel<util::Rastrigin<DIM>, DIM><<<1, N>>>(
        util::Rastrigin<DIM>(), 
        lower, upper,
        dX, dV,
        dPBestX, dPBestVal,
    ...
    // cleanup
    cudaFree(dX); cudaFree(dV);
    cudaFree(dPBestX); cudaFree(dPBestVal);
    cudaFree(dGBestX); cudaFree(dGBestVal);
    cudaFree(d_states);
}


TEST_CASE("pso::iterKernel inertia‐only updates X and V for 4 particles in 1D", "[pso][component][iter]") {
    ...
    double hPBX[N]; std::copy(hX, hX+N, hPBX);
    double hPBV[N]; std::fill(hPBV, hPBV+N, 1e6);
    double hGBX[DIM] = {0.0};
    double hGBV       = 1e6;
    ...
    pso::iterKernel<util::Rastrigin<DIM>, DIM><<<1, N>>>(
        util::Rastrigin<DIM>(),
        lower, upper,
        0.5,  // inertia
        0.0,  // cognitive
...
TEST_CASE("pso::iterKernel with zero w,c1,c2 leaves X unchanged and V zero", "[pso][iter]") {
    constexpr int N = 1, DIM = 2;
    const double lower = -5.0, upper = 5.0;
    const uint64_t seed = 42;
