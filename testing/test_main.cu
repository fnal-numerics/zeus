...
__global__ void
testVectorAddKernel(const double* a, const double* b, double* result, int size)
{
...
__global__ void
testVectorScaleKernel(const double* a, double scalar, double* result, int dim)
{
...
test_vector_add()
{
  ...
  cudaMemcpy(
    host_result, dev_result, size * sizeof(double), cudaMemcpyDeviceToHost);
...
test_vector_scale()
{
  ...
  cudaMemcpy(
    host_result, dev_result, dim * sizeof(double), cudaMemcpyDeviceToHost);
...
template <template <int> class Func, int DIM>
void
run_value_test(const double x[DIM], double expect, const char* name)
{
  double *dX, *dOut, hOut;
...
template <template <int> class Func, int DIM>
void
run_grad_test(const double x[DIM], const double expect[DIM], const char* name)
{
  double *dX, *dG, hG[DIM];
  cudaMalloc(&dX, DIM * sizeof(double));
  ...
  for (int i = 0; i < DIM; ++i) {
    std::cout << "expected: " << expect[i] << "\tactual: " << hG[i]
              << std::endl;
...
main()
{
  ...
  { // rastirign
    ...
    double x1[D] = {0.1, -0.3};
    double expected1 = 10.0 * D + (0.1 * 0.1 - 10 * cos(2 * M_PI * 0.1)) +
  ...
  // --- Ackley value tests (2D) ---
  {
    ...
    double expected1 =
      -20 * exp(-0.2 * sqrt(sumsq / D)) - exp(sumcos / D) + 20 + M_E;
  ...
  // --- Rosenbrock value tests (2D) ---
  {
    ...
    double x1[D] = {1.2, 1.2};
    double expected1 =
      100 * (1.2 - 1.2 * 1.2) * (1.2 - 1.2 * 1.2) + (1 - 1.2) * (1 - 1.2);
