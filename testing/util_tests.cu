...
extern "C" {
  __device__ void vector_add(const double* a,const double* b,double* r,int size);
  __device__ void vector_scale(const double* a,double s,double* r,int dim);
}

// kernels to invoke the the functions
__global__ void testVectorAddKernel(const double* a,const double* b,double* r,int n) {
  vector_add(a,b,r,n);
}
__global__ void testVectorScaleKernel(const double* a,double s,double* r,int n) {
...
TEST_CASE("vector_add works on small arrays", "[utils][vector]") {
  ...
  double *dA,*dB,*dR;
  cudaMalloc(&dA,N*sizeof(double)); cudaMalloc(&dB,N*sizeof(double)); cudaMalloc(&dR,N*sizeof(double));
...
TEST_CASE("vector_scale works for small arrays", "[utils][vector]") {
  ...
  double *dA,*dR;
  cudaMalloc(&dA,N*sizeof(double)); cudaMalloc(&dR,N*sizeof(double));
  cudaMemcpy(dA,hA,N*sizeof(double),cudaMemcpyHostToDevice);
